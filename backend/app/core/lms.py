...
def _student_from_record(record: Dict[str, Any]) -> Dict[str, Any]:
    email = record.get("email") or record.get("login")
    roll_no = record.get("roll_no") or record.get("student_id") or record.get("login")
    if not email or not roll_no or not record.get("name"):
        raise HTTPException(status_code=502, detail="LMS student profile is missing required fields")
    ...
    return {
        "lms_id": str(record.get("lms_id") or record.get("id")),
        "roll_no": str(roll_no),
        "name": record["name"],
        "department": str(department or "Unknown"),
        "semester": str(record.get("semester") or "Unknown"),
        "email": email,
    }
...
async def authenticate_student(email: str, password: str) -> Dict[str, Any]:
    ...
    return {**await get_student_details(session_id, uid), "session_id": session_id}
...
async def get_student_details(session_id: str, uid: Optional[int] = None) -> Dict[str, Any]:
    payload = {
        "jsonrpc": "2.0",
        "method": "call",
        "params": {
            ...
            "kwargs": {"fields": ["id", "name", "email", "login", "roll_no", "student_id", "department", "department_name", "semester", "lms_id"], "limit": 1},
        },
    }
    data = await _jsonrpc_post("/web/dataset/call_kw", payload, {"session_id": session_id})
    records = data.get("result") or []
    if not records:
        raise HTTPException(status_code=502, detail="Student profile not found in LMS")
    return _student_from_record(records[0])
